...
function scatters(frame,data){
    ...
    function drawChart(){
	    ...
	    var canvases = canvasHolder.selectAll("canvas.base").data([data]);
        canvases.enter().append("canvas.base").attr({
	    ...
	    var canvasTop = canvasHolder.selectAll("canvas.top").data([data]);
        canvasTop.enter().append("canvas.top").attr({
            width:(plotW * devicePixelRatio),
        	height:(plotH * devicePixelRatio)
        })
        .style({
  ...
  //  			context2.beginPath();
		...
		function highlightPath(player){
			...
			var canvi = d3.selectAll('#scatters canvas:nth-child(2)');

			canvi.each(function(d,i){
				var canvas = d3.select(this);
				var context = canvas.node().getContext("2d");

				context.clearRect(0,0,(plotW * devicePixelRatio),(plotH * devicePixelRatio));

				var cd = d.filter(function(a,b){
					return a.key == player
				})[0];

				var colScale = d3.scale.linear()
					.range(['#fff',player in oc(highlight) ? colours(player):'#43423e'])
					.domain([-5,cd.values.length-1]);

				cd.values.forEach(function(a,b){
					if(b != cd.values.length-1){
						context.strokeStyle = '#fff1e0';
						context.lineWidth = 5;
						context.beginPath();
						context.lineCap == 'round';
						context.moveTo(a.h, a.v);
						context.lineTo(cd.values[b+1].h, cd.values[b+1].v);
						context.stroke();
	   					context.closePath();

						context.strokeStyle = colScale(b);
						context.lineWidth = ((b+1)/(cd.values.length)*r*2);
						context.beginPath();
						context.lineCap == 'round';
						context.moveTo(a.h, a.v);
						context.lineTo(cd.values[b+1].h, cd.values[b+1].v);
